...
class SKFF(nn.Module):
    ...
    def forward(self, inp_feats):
        if not isinstance(inp_feats, (list, tuple)):
             raise TypeError("SKFF input must be a list of tensors [x, copy]")
...
if __name__ == "__main__":
    print(" Testing Final Model: Parallel + SKFF(r=2) + NoHIN")
    model = sfhformer_t()
    

    print(f"Parallel Transformer Check: {hasattr(model.layer1, 'transformer_branch')}")
    

    print(f"SKFF Check: {isinstance(model.skip1, SKFF)}")
    

    norm_type = model.layer1.cnn_branch[0].norm1.__class__.__name__
    print(f"Norm Type Check: {norm_type} (Should be BatchNorm2d)")
